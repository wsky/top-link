...
public class WebSocketChannelSelectHandler implements ChannelSelectHandler {
	private HashMap<String, ClientChannel> channels;
	...
	public ClientChannel getClientChannel(URI uri) throws ChannelException {
		...
		if (channel == null)
			channels.put(url, channel = this.connect(uri));
		return channel;
	}

	private ClientChannel connect(URI uri) throws ChannelException {
